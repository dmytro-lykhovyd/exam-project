...
class ProfilePage(BasePage):
    ...
    def go_to_profile_page(self):
        ...
        self.wait_until_clickable(locator=self.constants.USER_PROFILE_XPATH).click()
        return ProfilePage(self.driver)

    @log_wrapper
    def verify_profile_page_is_open(self):
        """verifies that profile page open"""
        create_account_button = self.wait_until_displayed(locator=self.constants.CREATE_ACCOUNT_BUTTON_XPATH)
        assert create_account_button.is_enabled()

    @log_wrapper
    def open_account_form(self):
        """opens fillable form for creating an account"""
        btn = self.wait_until_clickable(locator=self.constants.CREATE_ACCOUNT_BUTTON_XPATH)
        btn.click()
        radio_btn = self.wait_until_clickable(locator=self.constants.SEX_MALE_RADIO_BUTTON_XPATH)
        radio_btn.click()
    ...
    def create_account(self):
        ...
        terms = self.driver.find_element(by=By.XPATH, value=self.constants.TERMS_CHECKBOX_XPATH)
        # assert terms.is_enabled()
        terms.click()
